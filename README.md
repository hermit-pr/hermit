...
The Helm chart defaults to `image.tag` = `Chart.appVersion` (currently `0.2.7`), so the versioned image is used automatically.

For example:

```sh
docker pull registry.gitlab.com/hermit-bot/hermit:0.2.7
```
...
- `curl http://<service>/healthz` returns `{"status": "ok", "version": "0.2.7"}`.
...
**v0.2.7 is released.** The master, reviewer pods, providers, opencode integration, Helm chart, and all deployment assets are implemented and audited for production readiness.

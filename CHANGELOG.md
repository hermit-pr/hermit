...
- **JSON review verdicts emitted in an earlier message are no longer dropped.**
  `extract_text()` returned only the last opencode message's text, so a JSON
  verdict produced in an earlier turn was discarded before
...
[0.3.4]: https://gitlab.com/hermit-bot/hermit/-/compare/v0.3.3...v0.3.4
